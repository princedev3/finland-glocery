...
import { useForgotPasswordCheckMutation } from "@/app/_apis_/_user_index.api";
import { useEffect } from "react";
import { toast } from "sonner";

export default function VerifyEmail() {
const [forgotPasswordCheck]= useForgotPasswordCheckMutation()
  const router = useRouter()
  const searchParams = useSearchParams()
  const token = searchParams.get("token")
  useEffect(()=>{
const checkEmail = async()=>{
 const res = await forgotPasswordCheck({token})
if(res.data.status ===200){
    toast.success("verified")
    router.push(`/new-password?email=${res.data.message}`)
    return
}
